...
void Aimbot::FindTarget()
{
	while (true)
	{
		...
		/* if aimbot is enabled, and aimbot hotkey is held */
		if (settings->aimbot.enabled && GetAsyncKeyState(settings->aimbot.hotkey))
		{
			/* copy players into a local vector */
			globals->players.mutex.lock();
			std::vector<Player> local_players = globals->players.vec;
			globals->players.mutex.unlock();

			/* target selection */
			float closest_fov = settings->aimbot.fov;

			/* keeps track of iterations */
			int iterations = 0;

			/* iterate over players */
			for (const auto& ply : local_players) {
				...
				/* grab player object */
				auto player = (Player)(ply);

				/* ensure its not localplayer */
				if (player.local_player)
					continue;

				/* Check If Player Is Same Team s*/
				if (!settings->aimbot.attack_team && player.same_team)
					continue;

				/* This map will store screen position for bones */
				std::map<Bones, Vector2> screen_bones;

				/* tells us if we should continue drawing skeleton */
				...
				/* grab player's distance to cursor */
				float pythag_result = sqrtf(pow(((globals->ScreenSize.x / 2) - screen_bones[(Bones)settings->aimbot.bone_target].x), 2) + pow(((globals->ScreenSize.y / 2) - screen_bones[(Bones)settings->aimbot.bone_target].y), 2));

				/* ensure player is in fov */
				if (pythag_result < settings->aimbot.fov && closest_fov > pythag_result)
				{
					/* set target */
